...
anf i (App f es l)      = (i', stitch bs (App f es' l))
  where
    (i', bs, es')       = imms i es
-- anf should check if all the expressions in the tuple are immediate
-- similar to App
anf i (Tuple es l)      = (i', stitch bs (Tuple es' l))
  where
    (i', bs, es')       = imms i es
-- anf GetItem is anf when the two expresssions are immediate
...
-- immediate tuple should result in a pointer referring to the
-- start of the entire data structure
imm i (Tuple es l)      = (i'', bs', mkId x l)
  where
    (i', bs, vs)        = imms  i es
    (i'', x)            = fresh l i'
    bs'                 = (x, (Tuple vs l, l)) : bs
-- immediate (GetItem) should result in the value referred to by the
-- result of the underlying expression, so turn e1 into an immediate
-- as aresult of e2 turning into an immediate
