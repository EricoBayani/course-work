...
int window::move_speed = 4;
rgbcolor default_color{255,0,0};
rgbcolor window::border_color = default_color;
int window::border_size = 4;



// Implementation of object functions.
object::object (shared_ptr<shape> pshape_, vertex center_,
                rgbcolor color_, int label_):
  pshape(pshape_), center(center_),
  color(color_), label(label_) {}

void object::draw(bool isFocus) {
  pshape->draw (center, color,
                isFocus, to_string<int>(label));
}
...
void window::display() {
   ...
   for (auto& object: window::objects) {
     ...
     object.draw(object.focus);
   }
   objects[selected_obj].draw(true);
   mus.draw();
   glutSwapBuffers();
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      ...
      case 'H': case 'h':
        ...
        objects[selected_obj].move(-window::move_speed,0);
         break;
      case 'J': case 'j':
         //move_selected_object (
        objects[selected_obj].move(0,-window::move_speed);
         break;
      case 'K': case 'k':
         //move_selected_object (
        objects[selected_obj].move(0,window::move_speed);
      ...
      case 'N': case 'n': case SPACE: case TAB:
        // toggle current focus bool to false
        objects[selected_obj].focus = false;
        objects[selected_obj].draw(false); // redraw without border
        glutPostRedisplay();
        if(selected_obj < objects.size() -1)
          selected_obj = selected_obj + 1;
        else
          selected_obj = 0;
        // toggle new focus bool to true
        objects[selected_obj].focus = true;
        objects[selected_obj].draw(true); //redraw with border
        glutPostRedisplay();
         break;
      ...
      case '5': case '6': case '7': case '8': case '9':
        // toggle current focus bool to false
        if ((key - '0') < static_cast<int>(objects.size())){
          objects[selected_obj].focus = false;
          objects[selected_obj].draw(false); // redraw without border
          glutPostRedisplay();
          selected_obj = (key - '0');
